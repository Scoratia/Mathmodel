...
public class Q4Conv {
    ...
    static void caseRun(String name, int nel, double rtol, double tend, double dtout) {
        try {
            ...
            m.component("comp1").variable("var1").set("Rv", "R_i(t)");
    ...
    public static Model run() {
        ...
        double tend = 259200, dtout = 60;     // 72 h, 1 min
        p("=== 空间收敛（rtol = 1e-4）===");
        int[] nels = {40, 160, 640};
        for (int i = 0; i < nels.length; i++) {
            caseRun("m" + nels[i], nels[i], 1e-4, tend, dtout);
        }
        p("=== 时间收敛（Nel = 160）===");
        double[] rtols = {1e-3, 1e-4, 1e-5};
        for (int i = 0; i < rtols.length; i++) {
            caseRun("t" + (i + 1), 160, rtols[i], tend, dtout);
        }
